...
def llm(system, user, temperature=0.3, json_mode=False):
    kwargs = dict(
...
def analyze():
    ...
    try:
        ...
        structure = llm(STRUCTURE_PROMPT + "\n\n" + lang_instr, problem, temperature=0.2, json_mode=True)
        analogies = llm(ANALOGY_PROMPT + "\n\n" + lang_instr, f"Find analogies for this problem structure:\n\n{json.dumps(structure, indent=2)}", temperature=0.4)
        summary = [{"domain": a["domain"], "why_similar": a["why_similar"]} for a in analogies]
        synthesis = llm(SYNTHESIS_PROMPT + "\n\n" + lang_instr, f"Problem: {problem}\n\nAnalogies:\n{json.dumps(summary, indent=2)}", temperature=0.3, json_mode=True)
...
def deep_dive():
    ...
    try:
        ...
        result = llm(DEEP_DIVE_PROMPT + "\n\n" + lang_instr, json.dumps(payload, indent=2), temperature=0.3, json_mode=True)
